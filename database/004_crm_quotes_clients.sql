...
-- Cria orçamento diretamente do lead e reaproveita dados do evento.
create or replace function public.create_quote_from_lead(target_lead_id uuid)
returns uuid
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if not found or not private.is_company_member(source_lead.company_id) then
    raise exception 'Lead não encontrado';
  end if;

  insert into public.quotes (
    company_id, lead_id, created_by, assigned_user_id, title,
    event_type, event_date, event_city
  ) values (
    source_lead.company_id, source_lead.id, auth.uid(), source_lead.assigned_user_id,
    coalesce('Orçamento - ' || nullif(source_lead.name, ''), 'Orçamento'),
    source_lead.event_type, source_lead.event_date, source_lead.city
  ...
  values(source_lead.company_id, source_lead.id, auth.uid(), 'orcamento', 'Orçamento criado a partir do lead', jsonb_build_object('quote_id', new_quote_id));

  return new_quote_id;
end;
$$;

revoke all on function public.create_quote_from_lead(uuid) from public, anon;
grant execute on function public.create_quote_from_lead(uuid) to authenticated;

-- Converte o lead em cliente sem pedir novamente nome/telefone/e-mail/cidade.
...
begin
  ...
  where company_id = source_lead.company_id and source_lead_id = source_lead.id;
  ...
  update public.quotes set client_id = client_id, updated_at = now()
  where company_id = source_lead.company_id and lead_id = source_lead.id and public.quotes.client_id is null;

  update public.leads set status = 'ganho', won_at = coalesce(won_at, now()), updated_at = now() where id = source_lead.id;
  insert into public.lead_activities(company_id, lead_id, user_id, activity_type, description, metadata)
  values(source_lead.company_id, source_lead.id, auth.uid(), 'conversao', 'Lead convertido em cliente', jsonb_build_object('client_id', client_id));
